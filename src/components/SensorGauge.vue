...
export default {
  ...
  mounted() {
    ...
    if (this.hasRightType(this.updatedSensor.type)) {
      this.mountElements();
      //  this.updateValue(15);
      //  this.setValueAnimated(25, 2);
      setInterval(() => {
        return this.setValueAnimated(Math.floor(Math.random() * 10 + 1), 1);
      }, 3000);
    }
  },
  ...
  methods: {
    ...
    setGaugeColor(value, duration) {
      ...
      if (value < this.minRangeValue) {
        c = componentsSchemas.gauge.colors.success;
      } else if (value < this.maxRangeValue + this.minRangeValue / 2) {
        c = componentsSchemas.gauge.colors.success;
      } else if (value < this.maxRangeValue) {
        c = componentsSchemas.gauge.colors.alert;
      } else {
        c = componentsSchemas.gauge.colors.danger;
      }
